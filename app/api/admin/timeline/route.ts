...
export async function PUT(request: Request) {
  ...
  const { error: deleteError } = await supabase.from('timeline_entries').delete().not('id', 'is', null)
  if (deleteError) return NextResponse.json({ error: deleteError.message }, { status: 500 })

  if (parsed.data.length > 0) {
    const rows = parsed.data.map((entry, i) => ({ year: entry.year, events: entry.events, sort_order: i }))
